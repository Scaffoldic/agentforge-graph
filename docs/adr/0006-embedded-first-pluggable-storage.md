...
split on this — code-property-graph / data-flow tools and embedded
single-file indexers are embedded/zero-ops, while agent-oriented code
tools and server-based fact-indexing systems require a server.
Schema-driven / pluggable-adapter CKG designs resolved the tension with
...
3. **Pluggable adapters, embedded default** — Kuzu + LanceDB in a
   `.ckg/` dir by default; Neo4j / other graph-server backends as
   opt-in adapters.
...
the boring fallback; Neo4j and other graph-server backends are opt-in
pip packages registered via entry point. A `Store` facade owns one graph + one
...
- Switching to a shared graph-server backend (e.g. Neo4j) is
  config-only — no agent code changes.
...
- Research §2.6 (pluggable-adapter CKG designs — verified), §2.1
  (code-property-graph / data-flow tools embedded), §2.11 (SQLite
  trend), §5 storage recommendation.
